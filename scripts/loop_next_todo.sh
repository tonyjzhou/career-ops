#!/usr/bin/env bash
# Career-Ops — drain TODOS.md by running /next-todo once per item, EACH IN A FRESH CONTEXT.
#
...
# allowlist in scripts/loop_next_todo.settings.json — the career-ops dev toolchain (git/gh/make/
# node/npm/npx/go/docker/cops + safe shell builtins) is pre-approved and anything outside it is decided
# by auto mode's "safe yellow" judgment unattended rather than blocking on a prompt; secrets, sudo,
# curl/wget, rm -rf, platform secret-dumps (npm token) + registry auth/publish (npm login /
# npm publish / docker login / docker push), and destructive force-push
# (--force / -f) + hard-reset stay denied — but the SAFE
